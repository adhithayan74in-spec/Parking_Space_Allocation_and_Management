...
const updateSlot = async (req, res) => {
  try {
    ...
    const updated = await ParkingSpace.findByIdAndUpdate(id, req.body, {
      new: true,
      runValidators: true,
    });
    ...
    res.status(200).json({ message: "Slot updated", data: updated });
  } catch (error) {
...
const searchSlots = async (req, res) => {
  try {
    ...
    if (!query || !query.trim()) {
      return res.status(400).json({ message: "Please enter a pincode or address to search" });
    }
    ...
    const matchingSlots = await ParkingSpace.find({
      status: "available",
      $or: [{ pincode: regex }, { address: regex }, { locationName: regex }],
    });
...
const bookSlot = async (req, res) => {
  try {
    const { slotId, vehicleNumber, vehicleType, arrivalTime, leavingTime } = req.body;

    if (!slotId || !vehicleNumber || !vehicleType || !arrivalTime || !leavingTime) {
    ...
    if (isNaN(arrival.getTime()) || isNaN(leaving.getTime())) {
      return res.status(400).json({ message: "Invalid arrival or leaving time" });
    }

    if (leaving <= arrival) {
      return res.status(400).json({ message: "Leaving time must be after arrival time" });
    }
    ...
    res.status(200).json({ message: "Slot booked successfully", data: slot });
  } catch (error) {
...
const releaseSlot = async (req, res) => {
  try {
    ...
    if (slot.arrivalTime) {
      ...
      const durationHours = Math.max(0, (exitTime - slot.arrivalTime) / (1000 * 60 * 60));
      const fee = Math.ceil(durationHours) * (slot.pricePerHour || 20);
    ...
    res.status(200).json({ message: "Slot released successfully", data: slot });
  } catch (error) {
